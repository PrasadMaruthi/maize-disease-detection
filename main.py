...
def _find_last_conv_layer(model):
    """Find the last convolution-like layer in the loaded Keras model."""
    conv_types = (
    ...
    # Check nested models first/recursively so MobileNet-family backbones
    # are handled correctly when wrapped inside a classification model.
    def search_layers(layers):
        ...
        for layer in layers:
            ...
            if hasattr(layer, "layers") and layer.layers:
                found.extend(search_layers(layer.layers))
        return found

    conv_layers = search_layers(model.layers)
    if not conv_layers:
...
def make_gradcam(image, class_index):
    """Create a Grad-CAM heatmap and overlay for the predicted class."""
    model = load_model()
    last_conv_layer = _find_last_conv_layer(model)

    # Use the exact same preprocessing used by model_prediction().
    resized = image.convert("RGB").resize(IMAGE_SIZE)
    input_arr = np.asarray(resized).astype(np.float32)
    ...
    with tf.GradientTape() as tape:
        conv_outputs, predictions = grad_model(input_arr, training=False)
    ...
    # Resize heatmap to the original uploaded image dimensions.
    ...
    # Use the standard jet colour map for the Grad-CAM heatmap.
...
<div class="prediction-status">✓ &nbsp; Disease identified successfully</div>
                ...
                try:
                    gradcam_overlay, _ = make_gradcam(
                        image,
                        result_index
                    )
                    ...
                    with gradcam_col2:
                        st.image(
                            gradcam_overlay,
                            caption="Grad-CAM — Prediction-relevant regions",
                            use_container_width=True
                        )

                except Exception as gradcam_error:
                    st.warning(
                        "Grad-CAM visualization could not be generated for "
                        f"this model: {gradcam_error}"
                    )
