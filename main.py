...
def _find_gradcam_layer(model):
    # Prefer the last layer whose output is a feature map (4-D).
    # This also works when the CNN backbone is a nested model.
    ...
    # Fallback to convolutional layers.
    for layer in reversed(model.layers):
        if isinstance(
            layer,
            (
...
def _gradcam_visualization(test_image, predicted_index):
    ...
    try:
        ...
        # If the selected layer is not directly connected to the outer
        # model graph, use the last directly-connected 4-D layer.
        direct_layer = None
        ...
        grad_model = tf.keras.models.Model(
            inputs=model.inputs,
            outputs=[direct_layer.output, model.output]
        )
    ...
    # Convert the heatmap to a coloured image using PIL only.
