...
def _find_last_conv_layer(model):
    """Find the last convolutional layer, including inside nested Keras models."""
...
def _find_parent_model(root_model, target_layer):
    """Find the nested model that directly contains target_layer."""
...
def _gradcam_from_direct_graph(model, conv_layer, input_tensor, predicted_index):
    """Standard Grad-CAM when the convolutional layer is connected to model.inputs."""
...
def _gradcam_from_nested_model(model, conv_layer, input_tensor, predicted_index):
    """
    Grad-CAM fallback for models such as:
    [MobileNetV3Small/base_model, pooling, dense, ...].

    The convolutional layer is inside the nested feature extractor, so the
    outer model cannot directly be built with that layer's symbolic output.
    This function separates the feature extractor from the classifier.
    """
    parent = _find_parent_model(model, conv_layer)
    if parent is None or parent is model:
        raise ValueError("Could not identify the nested feature extractor.")

    # The parent model produces the feature extractor output.
    feature_model = tf.keras.models.Model(
        inputs=parent.inputs,
        outputs=[conv_layer.output, parent.output]
    )

    # First obtain the feature extractor input from the actual application
    # input, then run the complete model while keeping the feature tensor
    # watched. This handles the common nested CNN transfer-learning layout.
    with tf.GradientTape() as tape:
        tape.watch(input_tensor)

        # Run the complete model normally so prediction remains unchanged.
    ...
    # If the nested layer is not directly connected to the outer input,
    # use the nested feature extractor's own input graph and reconstruct
    # the classifier portion for the common Sequential/Functional layout.
    try:
        # For the common transfer-learning structure, the outer model is
        # Sequential and the nested feature extractor is followed by pooling
        # and Dense classifier layers. Rebuild only that classifier path.
        if isinstance(model, tf.keras.Sequential):
            ...
            with tf.GradientTape() as tape:
                conv_outputs, parent_output = feature_model(
                    input_tensor, training=False
                )
                tape.watch(conv_outputs)

                x = parent_output
                for layer in model.layers[parent_index + 1:]:
                    x = layer(x, training=False)

                if isinstance(x, (list, tuple)):
                    x = x[0]

                score = x[:, predicted_index]

            grads = tape.gradient(score, conv_outputs)

            if grads is not None:
                pooled_grads = tf.reduce_mean(grads, axis=(1, 2))
                conv_single = conv_outputs[0]
                pooled_single = pooled_grads[0]

                heatmap = tf.reduce_sum(
                    conv_single * pooled_single[tf.newaxis, tf.newaxis, :],
                    axis=-1
                )
                heatmap = tf.maximum(heatmap, 0)
                max_value = tf.reduce_max(heatmap)
        ...
        # Also try a directly connected classifier graph when available.
        post_model = tf.keras.models.Model(
        ...
        with tf.GradientTape() as tape:
            ...
            predictions_from_features = post_model(
                parent_output, training=False
            )
            if isinstance(predictions_from_features, (list, tuple)):
                predictions_from_features = predictions_from_features[0]
            score = predictions_from_features[:, predicted_index]
        ...
        if grads is not None:
            pooled_grads = tf.reduce_mean(grads, axis=(1, 2))
            heatmap = tf.reduce_sum(
                conv_outputs[0] * pooled_grads[0][tf.newaxis, tf.newaxis, :],
                axis=-1
            )
            heatmap = tf.maximum(heatmap, 0)
            max_value = tf.reduce_max(heatmap)
...
def generate_gradcam(test_image, predicted_index):
    """Generate the Grad-CAM overlay for the predicted class."""
    model = load_model()
    ...
    # Try the normal connected-graph Grad-CAM first.
    try:
        heatmap, layer_name = _gradcam_from_direct_graph(
            model, conv_layer, input_tensor, predicted_index
        )
    except Exception:
        # Fallback specifically for nested MobileNet/transfer-learning models.
        heatmap, layer_name = _gradcam_from_nested_model(
            model, conv_layer, input_tensor, predicted_index
        )
