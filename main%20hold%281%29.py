...
def _find_last_conv_layer(model):
    """Find the deepest convolutional layer, including nested CNN models."""
    # Search nested models first so a MobileNetV3Small/other backbone is
    # handled correctly even when it is stored inside the classifier.
    ...
    # Then search normal convolutional layers.
    for layer in reversed(model.layers):
        if isinstance(layer, (
...
def generate_gradcam(test_image, predicted_index):
    """
    Generate Grad-CAM using the ACTUAL loaded model forward pass.

    This deliberately does not construct a new Keras Model from an inner
    MobileNet layer to the outer classifier. That approach can fail for
    nested Functional models with Keras 3 because the symbolic tensors are
    owned by different graph scopes.

    Instead, the selected convolutional layer is temporarily instrumented
    so its real activation tensor is captured while the original model runs.
    Therefore the activation, prediction and gradient are guaranteed to be
    ...
    if last_conv_layer is None:
        raise ValueError(
            "Grad-CAM could not find a convolutional feature layer "
            "in the loaded model."
        )
    ...
    # Patch only this layer for this single forward pass.
    last_conv_layer.call = types.MethodType(capture_call, last_conv_layer)
    ...
    try:
        with tf.GradientTape() as tape:
            ...
            if len(captured) == 0:
                raise ValueError(
                    "The selected convolutional layer was not executed "
                    "during the model forward pass."
                )

            # If the layer is reused, the final call is the deepest/actual
            # activation that contributed to this prediction.
            conv_outputs = captured[-1]
            class_channel = prediction[:, predicted_index]

        grads = tape.gradient(class_channel, conv_outputs)

    finally:
        # Always restore the original layer, including when Grad-CAM fails.
        last_conv_layer.call = original_call

    if grads is None:
        raise ValueError(
            "Grad-CAM gradient could not be calculated. The selected "
            "feature layer is not connected to the prediction."
        )

    if len(conv_outputs.shape) != 4:
        raise ValueError(
            f"The selected feature layer produced shape {conv_outputs.shape}; "
            "Grad-CAM requires a 4-D feature map."
        )
...
def display_gradcam(test_image, predicted_index):
    """Display only the Grad-CAM image below the prediction results."""
    overlay = generate_gradcam(test_image, predicted_index)
